...
export default async function MessagesPage({ searchParams }: MessagesPageProps) {
  ...
  const otherName = active?.otherUser.profile ? `${active.otherUser.profile.firstName} ${active.otherUser.profile.lastName}` : messages.messaging.member;

  return <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col px-6 py-10"><h1 className="text-3xl font-semibold tracking-tight text-slate-900">{messages.messaging.messages}</h1><div className="mt-8 grid flex-1 overflow-hidden rounded-2xl border border-slate-200 bg-white md:grid-cols-[280px_1fr]"> <ConversationList conversations={conversations} activeId={active?.id} />{thread && active ? <ConversationThread conversationId={active.id} otherName={otherName} messages={thread.messages} currentUserId={user.id} /> : <div className="flex min-h-130 items-center justify-center bg-slate-50 px-6 text-center text-sm text-slate-500">{messages.messaging.noConversations}</div>}</div></div>;
}

function EmptyMessages({ message }: { message: string }) {
  return <main className="mx-auto flex w-full max-w-3xl flex-1 items-center justify-center px-6 py-16"><div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-8 py-12 text-center text-sm text-slate-600">{message}</div></main>;
}