...
export default async function MessagesPage({ searchParams }: MessagesPageProps) {
  ...
  return <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col px-6 py-10"><h1 className="text-3xl font-semibold tracking-tight text-slate-900">{messages.messaging.messages}</h1><div className="mt-8 grid flex-1 overflow-hidden rounded-2xl border border-slate-200 bg-white md:grid-cols-[280px_1fr]"> <ConversationList conversations={conversations} activeId={active?.id} />{thread ? <ConversationThread conversationId={active.id} otherName={otherName} messages={thread.messages} currentUserId={user.id} /> : <div className="flex min-h-[520px] items-center justify-center bg-slate-50 text-sm text-slate-500">{messages.messaging.startConversation}</div>}</div></div>;
}