    ...
    <div className="flex flex-1 flex-col bg-slate-50">
      <section className="border-b border-slate-100 bg-white">
        <div className="mx-auto w-full max-w-6xl px-6 py-10">
          ...
          <h1 className="mt-6 text-3xl font-semibold tracking-tight text-slate-900">{messages.directories.news}</h1>
      ...
      <section className="mx-auto w-full max-w-6xl flex-1 px-6 py-10">
        {articles.length === 0 ? (
          <p className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-sm text-slate-500">{messages.directories.noNews}</p>
        ) : (
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {articles.map((article) => (
              <article key={article.id} className="rounded-2xl border border-slate-200 bg-white p-6">
                ...
                <Link href={`/news/${article.slug}`} className="mt-5 inline-flex text-sm font-semibold text-emerald-700 hover:text-emerald-800">{messages.directories.read}</Link>
              </article>
            ))}
          </div>
        )}
