  ...
  return (
    <article id={`post-${post.id}`} className="rounded-2xl border border-slate-200 bg-white p-5">
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-4">
        ...
        <button type="button" onClick={() => setCommentOpen((open) => !open)} className="inline-flex items-center gap-2 rounded-full px-3 py-2 text-sm text-slate-600 hover:bg-slate-50"><MessageCircle className="h-4 w-4" aria-hidden />{post._count.comments} {messages.feed.comment}</button>
        <button type="button" onClick={sharePost} className="inline-flex items-center gap-2 rounded-full px-3 py-2 text-sm text-slate-600 hover:bg-slate-50"><Share2 className="h-4 w-4" aria-hidden />{messages.actions.save}</button>
      </div>
      {commentOpen && <div className="mt-4 space-y-3">{post.comments.map((comment) => <p key={comment.id} className="rounded-lg bg-slate-50 px-3 py-2 text-sm text-slate-700"><span className="font-semibold">{comment.author.profile ? `${comment.author.profile.firstName} ${comment.author.profile.lastName}` : messages.messaging.member}</span>{" "}{comment.content}</p>)}<form action={commentAction} className="flex gap-2"><input name="content" required maxLength={2000} placeholder={messages.feed.commentPlaceholder} className="min-w-0 flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm" /><button type="submit" disabled={commentPending} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">{messages.feed.comment}</button>{commentMessage && <span className="sr-only">{commentMessage}</span>}</form></div>}
    </article>
  );
}