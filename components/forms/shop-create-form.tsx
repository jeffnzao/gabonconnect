...
export default function ShopCreateForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(submitError instanceof Error ? submitError.message : "Unable to create the shop.");
    } finally {
    ...
    <div className="flex flex-1 flex-col bg-slate-50">
      <section className="border-b border-slate-100 bg-white">
        <div className="mx-auto w-full max-w-3xl px-6 py-10">
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-900">Create a shop</h1>
          <p className="mt-2 text-sm text-slate-500">List a shop, service, or local offering for the GabonConnect community.</p>
        </div>
      </section>
