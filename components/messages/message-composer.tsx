...
export default function MessageComposer({ conversationId }: { conversationId: string }) {
  ...
  return (
    <form action={formAction} className="flex gap-2 border-t border-slate-200 bg-white p-4">
      <input name="content" required maxLength={5000} placeholder="Write a message" className="min-w-0 flex-1 rounded-full border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-emerald-400" />
      <button type="submit" disabled={isPending} className="rounded-full bg-emerald-500 px-5 py-2.5 text-sm font-semibold text-slate-950 disabled:opacity-60">{isPending ? "Sending..." : "Send"}</button>
