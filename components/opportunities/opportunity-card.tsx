...
export default function OpportunityCard({ opportunity }: OpportunityCardProps) {
  const issuer = opportunity.association?.name ?? opportunity.companyName ?? "GabonConnect member";
  return (
    <article className="flex flex-col rounded-2xl border border-slate-200 bg-white p-6">
      ...
      {opportunity.isRemote && <p className="mt-2 flex items-center gap-2 text-sm text-slate-600"><Wifi className="h-4 w-4 text-emerald-600" aria-hidden />Remote available</p>}
      <p className="mt-4 text-sm text-slate-500">Posted by {issuer}</p>
      <p className="mt-2 text-xs text-slate-400">{opportunity.createdAt.toLocaleDateString("en-US")}</p>
      <Link href={`/opportunities/${opportunity.slug}`} className="mt-5 inline-flex text-sm font-semibold text-emerald-700 hover:text-emerald-800">View opportunity <span className="ml-1" aria-hidden>→</span></Link>
    </article>
  );
}