...
export type FeedPreferences = { memberStatus: MemberStatus; cityName: string | null; countryName: string | null; interests: string[] };
...
export const getPersonalizedFeed = cache(async () => {
  ...
  return { preferences, sections: await getPersonalizedFeedForPreferences({ memberStatus: preferences.memberStatus, cityName: preferences.city?.name ?? null, countryName: preferences.city?.country.name ?? preferences.country?.name ?? null, interests: preferences.interests }) };
});

export async function getPersonalizedFeedForPreferences(preferences: FeedPreferences) {
  const [articles, events, opportunities, procedures, scholarships] = await Promise.all([
    ...
    prisma.administrativeProcedure.findMany({ where: { moderationStatus: "APPROVED", publishedAt: { not: null }, archivedAt: null }, take: 10, select: { id: true, title: true, slug: true, category: true } }),
  ...
  const city = preferences.cityName;
  const country = preferences.countryName;
  const interests = preferences.interests;
  const matches = (text: string) => interests.some((interest) => includes(text, interest));
  const hasInterest = (value: string) => interests.some((interest) => includes(interest, value) || includes(value, interest));
  ...
  return {
    ...
    procedures: procedures.filter((item) => matches(`${item.title} ${item.category}`) || preferences.memberStatus === "STUDENT" || (item.category === "CONSULAR" && hasInterest("consulat")) || (item.category === "IMMIGRATION" && hasInterest("visa"))).map((item) => ({ id: item.id, title: item.title, detail: item.category, href: `/procedures/${item.slug}`, reason: "Guide utile pour votre profil" })),
